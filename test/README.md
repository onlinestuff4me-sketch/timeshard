...
| `duelmeet.mjs` | Meeting a new type stops the world. Walks the run door by door to each debut room — `warpDoor` moves the room number and not the fight in front of you — and checks that the type's first act freezes time, that the card is its NAME pinned to its body, that a thumb shows which way to go, and that dodging is what lets the world move again. Once per type, not once per body. |
