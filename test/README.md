...
| `duelwalk.mjs` | A capture, not a check, and skipped unless named. Stands in each of the first twelve NO RETREAT rooms from the first frame to the last body and records the strip, every man who arrives, what he is, when, and how far away. The data behind `docs/DUEL.md` and the published range card. |
