...
| `duelwalk.mjs` | A capture, not a check, and skipped unless named. Stands in each of the first twelve NO RETREAT rooms from the first frame to the last body and records the strip, every man who arrives, what he is, when, and how far away. It is the floor-plan capture; the schedule in `docs/DUEL.md` comes from `duelramp.mjs`, which reads it out of the running game. |
...
| `duelmeet.mjs` | Meeting a new type stops the world. Walks the run door by door to each debut room — `warpDoor` moves the room number and not the fight in front of you — and checks that the type's first act freezes time and fills the screen with three stacked rows: its NAME in the announcing type, the onboarding's own DODGE line, and a thumb travelling the way to go. The rows must be in order, on screen, headline biggest, and neither text row may run off the sides. Dodging is what lets the world move again, once per type and not once per body. |
| `duelbtn.mjs` | NO RETREAT's time button: absent in room 1, nothing slowing itself there, and then present — in whichever room the schedule hands it over, which the probe walks to rather than assuming — wearing its own charge with the big meter, drained while spending, refilled on kills, letting go by itself when the bank empties. |
