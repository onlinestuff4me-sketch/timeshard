...
| `loadout.mjs` | (NO RETREAT, which keeps one gun.) WHAT YOU ARE CARRYING ONLY EVER GETS BETTER BY WALKING. Drops a weapon at the player's feet and stands on it. A pistol clip must not take the shotgun away — `setWeapon` on a clip is a SWAP when you are not holding the pistol — and the declined drop stays on the floor rather than being silently eaten. An upgrade is still an upgrade, a clip still arms a player holding only a knife, and the same weapon again is ammo rather than a downgrade. |
...
| `runlog.mjs` | THE RUN LOG. A run from the menu is recorded (a kill, a miss, the pause, the death and its cause) and saved to disk. RUN LOG on the pause menu opens the panel without the same tap pressing a button in it; its summary counts the kill and 50% accuracy; SEND TO GITHUB is a new issue on the repo carrying the typed note, under 8,000 characters; CLOSE returns to the pause menu. After a death the log is still on disk, the death screen shows SEND RUN LOG, and tapping it opens the panel instead of starting a retry. |
