...
| `duel.mjs` | CORRIDOR DUEL can be finished: 40 seconds of shattering everything that appears must field a wave, open doors and carry the player forward. It scored 2 bodies, 0 doors and z 0 before the placement fix, and deadlocks — floor empty, bodies owed, door shut — are counted and capped. |
| `duelbtn.mjs` | The duel's time button: absent in room 1 (the introduction), present in room 2 wearing its own charge with the big meter, drains while spending, refills on kills, and lets go by itself when the bank empties. |
