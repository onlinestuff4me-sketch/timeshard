...
| `duelbtn.mjs` | NO RETREAT's time button: absent in room 1, nothing slowing itself there, and then present — in whichever room the schedule hands it over, which the probe walks to rather than assuming — wearing its own charge with the big meter, drained while spending, refilled on kills, letting go by itself when the bank empties. Pressing it takes the prompt down and leaves nothing in its place but the shooting cue — a ring on a man and a thumb on his chest, in the slowed room — because the world has just stopped and nothing has said what to do with a stopped world. The refill line is NOT here: it waits for the bank to have been spent, and `duelcoach.mjs` is where that is checked. |
| `duelcoach.mjs` | **Three things the opening of NO RETREAT says, and when.** All three came out of one playtest and all three are a message arriving at a moment that makes it unreadable or untrue. `STARTING SIMULATION` had no clock — it held until the first round was fired, so the men walked on and aimed underneath it and it landed in the same band of pixels as the door's own `EXIT`; it is two seconds now, and the probe reports how long it was actually readable and whether a shot or its own clock took it down. `TAP TO SLOW TIME` arrives with the world stopped and an arrow on a button, and you shoot here by tapping ANYWHERE — so the probe taps twice away from the button and the magazine must not move. And `SHATTERING REFILLS YOUR METER` used to be said one frame after the first press, with the bank still full and nothing spent; it now waits for the bank to run low and be paid back with three bodies, and is read ON THE FRAME IT IS RAISED, because the kills that pay the bank back are the kills that empty the room. |
| `doorframe.mjs` | **Is the wall beside a door actually closed?** The exit door's jambs were 0.7 m because that reaches from the opening's edge to the cell's side wall — right whenever the end cell has one. The simplified modes' rooms are an authored strip widened a cell either side for their whole length, door row included, so there was no side wall and the jamb stopped 0.3 m short of anything: a slot floor to ceiling, both sides, every room of NO RETREAT and of `stop`. The vault's interior lintel had the same bug in the other axis, 45 mm short of the ceiling across both its doorways. Asks by shooting a z-ray across the boundary at every (x, y) rather than sampling the plane — jambs sit centred on the cell boundary and the walls either side sit 0.15 m inside it, so plane-sampling reports holes that are not there. No browser; it rebuilds `buildHallLeg`'s wall boxes, which is the one thing in it that has to be kept in step by hand. |
| `spine.mjs` | **How far along the path the player has walked.** Walks a leg shaped like the opening sequence — a T-junction whose right arm is a dead end — at real speed, one frame at a time, down three differently-shaped branches, and runs the old nearest-cell rule beside the current one. The old rule reported spine cell 26 and 28 where the player had walked as far as 18, silently and permanently, because it is monotonic. No browser: the measure is arithmetic over the leg's own cells. |
| `joke.mjs` | **The dead end's man, and the death that is a joke.** One arm of the opening T is a gag: two painted warnings, then somebody with his arm already up. He belongs to the BRANCH rather than the leg — placed on entry, taken away on exit, because `cleared` advances on `!enemies.length` and a man left standing round two corners is a shooting lesson that can never finish. Then the round: one cell in from the corner, holding a perfect input from the first frame, standing dies at 0.9 s, running back out at 1.0 s and both sidesteps at 0.8 s. And the death itself — no card, no button, no run filed, and back on the approach facing the sign that was ignored. |
| `faced.mjs` | **The looking lesson ends when they have looked.** `corners` runs to the last corner because that is where the barrier comes into view, but its PROMPTS must not: the junction is four cells earlier and `DRAG TO MOVE` landed across `THIS WAY`. They retire on `faced` — standing at the corner the second jog rejoins the axis at, turned to look down the hallway. Checks they are up through the corners, that arriving is not enough, that turning retires both them and the divider, and that a joke death re-entering the step does not flash them for a frame. Also walks all seven lessons and asserts the way-out needle is down and ungranted on every one. |
| `exitsign.mjs` | **`EXIT` above every door of the run.** The message system belonged to a lesson and belongs to a leg; four separate gates kept it out of the game, the last of them `#ts-sign` living inside a `display:none` container. Checks a run leg reads the run's spec, the sign is drawn at a readable size while walking at the door it names, it survives a crossing, building it costs under a tenth of a frame, and the `DOOR N` card stands down for it. Then the hand-off: the needle carries the corridor while the sign is a smudge, retires once it is readable, and comes back when the player turns their back on it. |
| `story.mjs` | **The script, checked against the document it was transcribed from, and placed.** No browser. Per-act counts, that only the programme's lines decode, that every line fits three rows on a phone, that no beat carries a door number, and that a rewrite is a jam rather than a redraft. Then the placement across seven difficulty ramps: strictly increasing doors, the placed beats a subsequence of the script, the engineers' line exactly on the unlock and the win condition exactly on the last door. Prints the decode log and what the script needs from the ramp. |
