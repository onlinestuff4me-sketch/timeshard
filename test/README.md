...
| `meetcard.mjs` | THE DEBUT CARD, in the tunnel. A type this save has never met stops the world as he finishes assembling: the camera settles on him with the lens closed (he sits centred in the top half), the room is dimmed, and the panel names him, says what he does, and gives a hint only where it is not obvious (the heavy has none). The world clock does not move under it; a tap takes it down without firing, and the camera and controls come back; the same type is not carded twice, not after a reload either; and the gunner is never carded. |
