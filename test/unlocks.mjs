...
const head = await page.$eval('#unlocks h2', (n) => n.textContent);
if (head !== 'UNLOCKS') bad('the screen is still called ' + head);
const games = await page.evaluate(() => {
  const secs = [...document.querySelectorAll('#unlocklist .asec')];
  const g = secs.find((s) => /^MODES/.test(s.textContent));
  if (!g) return null;
  const rows = [];
...
if (!games) { bad('UNLOCKS has no MODES section'); }
else {
  console.log('modes  ' + games.head);
