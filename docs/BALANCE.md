...
|---|---|---|---|
| head | 1.6× | 3.2 s | 1.98 s |
| chest | 1× | 2 s | 1.24 s |
| pelvis | 0.9× | 1.8 s | 1.12 s |
| legs | 0.7× | 1.4 s | 0.87 s |
| blast / no wound | 1× | 2 s | 1.24 s |

---

## The shatter

52 pieces per body from a pool of 512 instanced slots
(materialising enemies draw on their own pool of 768, so a
...
so an edge is 0.196 × scale. Two classes in every burst: 34%
are chunks big enough to follow with your eye, and the rest is grit.

| Knob | Value | Meaning |
|---|---|---|
| `chunkFrac` | 0.34 | share that are big pieces |
| `chunkMin` / `chunkVar` | 0.5 / 0.9 | chunk scale range |
| `gritMin` / `gritVar` | 0.15 / 0.36 | grit scale range, weighted small by `gritCurve` 2.2 |
| `speedBase` / `speedVar` | 2.2 / 3.2 | m/s, most slow with a fast tail |
| `impulse` / `impulseVar` | 1.6 / 2 | shove along the killing blow |
| `rise` | 1.5 | upward bias, so the cloud sits above the wound |
| `drag` / `spin` | 0.6 / 10 | settle rate, rad/s per axis |
| `life` / `lifeVar` | 2.8 / 1 s | seconds on screen |
| `breakWindow` | 0.05 s | world time a single zone crumbles over |

### The per-part break

The body is four zones. The zone you hit throws **1.6× its share**
of the pieces at **1.45× the speed**, and the rest follow outward
at **0.05 s of world time per zone away from the wound** — so a
...
|---|---|---|---|
| head | 1.44 – 1.82 | 16% | ±0.2 m |
| chest | 1.02 – 1.44 | 34% | ±0.32 m |
| pelvis | 0.74 – 1.02 | 20% | ±0.27 m |
| legs | 0.04 – 0.74 | 30% | ±0.24 m |
