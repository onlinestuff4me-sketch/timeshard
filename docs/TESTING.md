...
| `saves.js` | the save list end to end: a first launch has one action and no list; depth makes the button say CONTINUE and name the door; **continuing actually starts on that door** (it never used to — see docs/SAVES.md); NEW RUN is a separate save at door 1 that leaves the old one alone; delete asks in the row, KEEP keeps, confirm removes; and deleting the last save leaves a menu that still starts a run |
